...
function Server() {
    var oo = this;
    oo.get_action1 = function (m)		//开始搜索博弈树
    {
        var action = fetch_action1(m);
        console.log("server1结果：", action);
        return action;
    }

    oo.get_action2 = function (m)		//开始搜索博弈树
    {
        var action = fetch_action2(m);
        console.log("server2结果：", action);
        return action;
    }

    function fetch_action1(m) {
        var action
        var json_data = {'board': m, 'nexts': m.nextIndex};
        // var json_data = {'board':[12,2],'nexts':[12]};
        json_data = JSON.stringify(json_data)
        $.ajax({
            type: 'POST',
            url: Flask.url_for('reversi_app.get_action1'),
            data: json_data,
            ...
            error: function (err) {
                ...
                action = m.nextIndex[Math.random() * m.nextIndex.length >> 0];
            }

        });
        return action
        // console.log('server next move:',m.nextIndex);

    }

    function fetch_action2(m) {
        var action
        var json_data = {'board': m, 'nexts': m.nextIndex};
        // var json_data = {'board':[12,2],'nexts':[12]};
        json_data = JSON.stringify(json_data)
        $.ajax({
            type: 'POST',
            url: Flask.url_for('reversi_app.get_action2'),
            data: json_data,
            contentType: 'application/json',
            dataType: 'json',
            async: false,
...
function Othello() {
    ...
    oo.serverSide = 0; //1: server为黑棋,  -1: server为白棋,  0: 没server的事情  2: server自己对战

    var aiRuning = false;	//AI运算中...
    var aiRuningObj = document.getElementById("airuning");
    var serverRuningObj = document.getElementById("serverruning");
...
//        var aiAuto = oo.aiSide == map.side || oo.aiSide == 2;
        ...
        setServerRunStatus(false);
        setPassStatus(false);
        board.update(map, false);
...
//                timer = setTimeout(aiRun, 50);
                timer = setTimeout(ServerRun1, 50);
            }, 400);
        } else {
            // aiRuning = false;
            timer = setTimeout(function () {
                setServerRunStatus(true);
                timer = setTimeout(ServerRun2, 50);
            }, 400);
        }
...
//    }
    function ServerRun1() { //server走棋
        oo.go(server.get_action1(map));
    }

    // document.getElementById("ai").onclick = aiRun;
    function ServerRun2() { //server走棋
        oo.go(server.get_action2(map));
    }
    ...
    function setServerRunStatus(t)		//设置Server运算状态
    {
        serverRuningObj.style.display = t ? "block" : "none";
    }
