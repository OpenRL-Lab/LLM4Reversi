    ...
    */
    ...
    if ($('.swiper-horizontal').length) {
        ...
        $('.swiper-horizontal').on('mouseenter', '.swiper-slide', function () {
            $('figcaption', this).stop().animate({opacity: 1}, 300, 'easeInOutQuad', function () {
            }).end();
        });
        $('.swiper-horizontal').on('mouseleave', '.swiper-slide', function () {
            $('figcaption', this).stop().animate({opacity: 0}, 300, 'easeInOutQuad', function () {
            }).end();
        });
...
function appendGmapApi() {
    ...
    if ($('#map-wrapper').length) {
        ...
        script.src = "http://maps.google.com/maps/api/js?key=AIzaSyB6_ZcwSicHc1QoU0x8gl3rL2DpKKeWzjQ&sensor=true&callback=initialize";
